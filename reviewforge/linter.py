...
def basic_lint(rel_fname, code):
    ...
    try:
        ...
        lang = filename_to_lang(rel_fname)
        if not lang:
            return
        if lang == "typescript":
            return
...
class Linter:
    ...
    def run_cmd(self, cmd, rel_fname, code):
        ...
        try:
            result = subprocess.run(
                ...
                cwd=self.root, encoding=self.encoding, errors="replace"
            )
    ...
    def errors_to_lint_result(self, rel_fname, errors):
        ...
        if filenames_linenums:
            _, linenums_set = next(iter(filenames_linenums.items()))
            linenums = [num - 1 for num in linenums_set]
    ...
    def flake8_lint(self, rel_fname):
        ...
        flake8_cmd = [
            ...
            f"--select={fatal}", "--show-source", "--isolated", rel_fname,
        ]
        ...
        try:
            result = subprocess.run(
                flake8_cmd, capture_output=True, text=True, check=False,
                encoding=self.encoding, errors="replace", cwd=self.root,
            )
