...
from reviewforge.watch_prompts import watch_ask_prompt, watch_code_prompt


def load_gitignores(gitignore_paths: list) -> Optional[PathSpec]:
...
class FileWatcher:
    """Watches source files for AI comment markers and triggers the coder."""
    ...
    def filter_func(self, change_type, path):
        """Filter function for the file watcher — only respond to files with AI markers."""
        path_obj = Path(path)
        path_abs = path_obj.absolute()

        try:
            path_abs.relative_to(self.root.absolute())
        except ValueError:
            return False
    ...
    def process_changes(self):
        ...
        for fname in self.changed_files:
            ...
            rel_fname = self.coder.get_rel_fname(fname) if hasattr(self.coder, "get_rel_fname") else fname
            if not added:
                self.io.tool_output()
                added = True
        ...
        for fname in self.coder.abs_fnames:
            ...
            try:
                ...
                context = TreeContext(
                    ...
                    mark_lois=True, loi_pad=3, show_top_of_file_parent_scope=False,
                )
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if match := self.ai_comment_pattern.search(line):
                ...
                if comment:
                    ...
                    c = comment.lower().lstrip("/#-;").strip()
                    if c.startswith("ai!") or c.endswith("ai!"):
                        has_action = "!"
                    elif c.startswith("ai?") or c.endswith("ai?"):
                        has_action = "?"
